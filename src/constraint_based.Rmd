...
Read in the data and create subset
```{r}
data <- read.csv(picked_data, header=FALSE, skip=1)
dsub_set <- subset(data, select=-(1:2))
dsub_set$V3 <- as.numeric(factor(dsub_set$V3, levels = unique(dsub_set$V3)))
```

Apply all the constraints-based algorithms (IAMB variations)
